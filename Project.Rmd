...
```{r}
predictions_ = predict(model_RegTrp, newdata=testing_)
predictions_
...
```{r}
rmse_ <- sqrt(sum((predictions_ - testing_$VALUE)^2)/nrow(testing_))
rmse_
```


```{r}
R2 <- cor(testing_, predictions_)^2
R2
```
...
model_T <- train(value~ref_date+sex+ age_at_admission+years_since_admission+income_type+statistics+uom, 
...
rmse1 <- sqrt(sum((predictions_1 - testing1$value)^2)/nrow(testing1))
rmse1
```


```{r}
r2 <- cor(testing1, predictions_1)^2
r2
```
...
rmse2 <- sqrt(sum((predictions_2 - testing1$value)^2)/nrow(testing1))
rmse2
```
...
```{r}
r3 <- cor(testing1, predictions_2)^2
r3
